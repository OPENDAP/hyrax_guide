...
links to Java WebStart applications and to WebService
...
location outside the _$CATALINA_HOME/webapps/opendap_ directory, as
...
web.xml. For example, add the _ENTITY_
...
to the _DOCTYPE_ declaration at the top of the _web.xml_, and
also add an _entity reference_ (_securityConfig_, as above)
