...
and "time". This is suggestive, but not as helpful as one could wish. To find out more about what the data __is__, you can look at the other
...
right longitude, __other expressions__)", ehere _other expressions_ must be enclosed in double quotes, and can be one of these forms:
...
text files. Essentially, the person or organization providing data does not need to do anything besides link:../index.php/Hyrax[installing the server].
