...
// jhrg TODO There should be a better way to write the [] in the URLs. People will think they have to use the hex escapes.
...
// jhrg TODO Add a note that other servers may support other ways of doing an equivalent operation (e.g., ERDDAP does, TDS does not AFAIK). We don't really want to rank on TDS, however, so maybe soft-pedal that...
...
// jhrg TODO Google works too. Use Google to search for 'OPeNDAP Hyrax' or to search for 'OPeNDAP <terms>' or 'Hyrax <terms>'
