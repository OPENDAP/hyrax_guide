...
* *Gateway:* For more imformation, see the documentation for 
  link:../Hyrax/handlers/Gateway_Service.adoc[Gateway Service].
...
//IS THIS A VALID LINK? -ACP
For information about logging, please check out the 
link:./Hyrax_Logging_Configuration.com[Hyrax Logging Configuration Documentation].
...
to the <__!DOCTYPE__> declaration at the top of the _web.xml_, and also
add an __entity reference__ (`&securityConfig;`) 
to the content of the <__web-app__> element. This would cause your external
security configuration to be included in the _web.xml_ file.

===== Example _ENTITY_ configuration:
...
Apache to deliver compressed responses]. Tomcat will not compress
