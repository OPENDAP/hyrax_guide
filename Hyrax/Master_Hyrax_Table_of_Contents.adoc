...
// TODO Make this a multi-part book? Maybe... To do that with the content we have, raise the levels up one notch. That means reduce the leveloffset values by one (+1 --> 0, +2 --> +1) and the sections in this document by one ('==' --> '=', ...).
//
...
[appendix]
include::BES_XML_Commands.adoc[leveloffset=+"]
