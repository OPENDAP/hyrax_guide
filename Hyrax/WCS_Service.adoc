...
DynamicService. In the example above the `pathMatch` attribute is set like this:
+
...
interprets the value as a URL pointing to a Hyrax server and will proceed to utilize the remote server to provide metadata and data required to perform the WCS activities. The service can be configured to authenticate for data access as necessary.
