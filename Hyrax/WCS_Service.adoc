...
*; the _range_ of
  the coverage is the values of the variable. These values must have an
...
commit development resources to it. Please contact us if you are
interested by sending a note to
...
variables will make suitable coverages, or it may just not be in an
...
_DynamicService_ element. Consider the following _DynamicService_ definition:

.Each DAP dataset in the variable is exposed as a WCS field and basic information required by WCS is provided
[source,xml]
----
<DynamicService
        prefix="coads"
...
NOTE: When creating filed defintions remember that WCS Field names
have limitations on the kinds of characters they can contain.
Specifically, these field names must be
https://stackoverflow.com/questions/1631396/what-is-an-xsncname-type-and-when-should-it-be-used[NCNAMEs],
whci means that they cannot contain special sysmbols such as :, @, $,
%, &, /, +, pubtuation, spaces, tabs or newlines or different
parenthesis. Furthermore, they cannot begin with a digit, dot (.) or
minus (-) although those can appear later in the name.
