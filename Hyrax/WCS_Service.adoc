...
=== Using _pathMatch_
The `pathMatch` attribute is used to assign a WCS DynamicService definition to some subset (or possibly all) of the Datasets available through the Hyrax server. This is accomplished by applying a the regular expression contained in the value of the `pathMatch` attribute to the _local name_ (aka _local url_, _path part of url_, etc. ) of a candidate dataset.
