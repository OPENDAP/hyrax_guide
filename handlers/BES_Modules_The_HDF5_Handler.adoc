...
----
An example:
HDF5 variable name __velocity.u__ under group __geo-location__  
becomes the __geo_location_velocity_u__ in the DAP output. 
----
 ...
 * Tune its behavior by changing the value and the H5.CachePurgeLevel value below. Note that
...
You do not need to edit this to use the cache.  
 ...
 **  By default, this key is set to __true__. If the fillvalue is out of the range of the variable type, 
 DAP service issues an error and stops. 
 ...
 * When this key is set to __true__, the handler will check if there exists name clashing among variables and attributes. If name clashing occurs, the handler tries to resolve the
 ...
 So to reduce performance overhead, we set this key to __false__ by default. 
 Users can set this key to __true__ if it becomes necessary.
 
H5.NoZeroSizeFullnameAttr:::
* When this key is set to __true__, the fullnamepath attribute will NOT be added if the HDF5 data storage size is 0. This is necessary to generate correct HDF5 DMRRPP files.
...
* Since Hyrax 1.15, MetaData Store(MDS) has the similar feature as this key achieves. By default, this key is set to __false__. Users are encouraged to check if turning this key on can improve performance before setting this key __true__.
...
* NOTE: When HDF-EOS5 level 3 Grid products are served by Hyrax, turning on this feature may
  ...
  this feature is the served data products are HDF-EOS5 level 3. Since several BES Keys are involved 
  when this feature is turned on, by default this key set to __false__.
 ...
 * If this key is set to __true__, the variable data will write to a binary file in the
  ...
  ** H5.DiskCacheFilePrefix - The cache file prefix, it must be given
  for the cache to work. This requirement comes from the BES.
...
H5.DiskCacheComp:::
  ...
  * NOTE: This key will take effect only when the __H5.EnableDiskDataCache__ key to __true__. 
  ...
  *** H5.DiskCacheCompThreshold: The value of this key must be a floating-point number that is greater than 1. 
...
When this key is set to __true__F, the handler follows the netCDF-4 data model to handle the HDF5 dimensions if possible.
...
* For DAP2, the mappings of 64-bit integer, time, enum, bitfield, 
        opaque, compound, array, and reference types are not supported.  For DAP4, the mapping of HDF5 64-bit integer datatype is supported. But the other datatypes are still unsupported.
...
* HDF5 products
** SMAP, GPM, OCO2/ACOS/GOSAT, AirMSPI, Aquarius 
...
IMPORTANT: After checking the ignored HDF5 object and attribute information, make sure change the CheckIgnoreObj key back to _false_.  **H5.CheckIgnoreObj=false**
