...
The BESManager information is used whenever the software needs to access the
BES's services. This configuration is key to the function of Hyrax, for in it
is defined each BES instance that compose a given Hyrax installation. The
following examples will show a single BES example. For more information on
configuring Hyrax to use multiple BES's
<<config-hyrax-mult-BESs, look here>>.

Within the BESManager element there are 3 principle child elements:

1. The BES element(s) (one or more required).
1. The NodeCache element (optional).
1. The SiteMapCache element (optional).

===== `<BES>` element (required)

Each BES component of hyrax is identified using a seperate `<BES>` child
element inside of the `<BESManager>` element.
This `<BES>` element provides the OLFS with connection and control
information for a BES. There are 3 required child elements within a `<BES>`
element: `<prefix>`, `<host>`, `<port>`, and 4 optional child elements:
`<timeOut>`, `<maxResponseSize>`, `<ClientPool>`, and `<adminPort>`.
...
The `maxCmds` attribute specifies the maximum number of commands that can be
issued over a particular BESClient connection. The default is 2000.
...
If the `<ClientPool>` element is missing, the pool (`maximum`) size defaults to
200 and `maxCmds` defaults to 2000.
