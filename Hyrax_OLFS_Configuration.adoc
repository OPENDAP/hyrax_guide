...
front end service. What follows is a detailed explantion of the files content.

==== `<OLFSConfig>` Element

The <__OLFSConfig__> element is the document root. It contains several
elements that suppy the configuration for the OLFS.

==== `<BESManager>` Element (required)

The `BESManager` element provides configuration for the BESManager class.
The BESManager is used whenever the software needs to access the BES's services.
