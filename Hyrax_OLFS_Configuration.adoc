...
of an existing directory that is readable and writable by Tomcat, it is
used.
Otherwise,
. If the directory `/etc/olfs` exists and is readable and writable by
Tomcat user the OLFS will use it. Otherwise,
...
an *SELinux* variant, the `/etc/olfs` location will not work. However the
