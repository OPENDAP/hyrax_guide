...
The  `<UseDAP2ResourceUrlResponse />` key has been deprecated use
...
The DataRequestForm element defines three server behaviors:
...
=== `<EnableCombinedLog />` element (optional)

This optional element, when enabled, will cause the OLFS to send
...
=== `<NoDynamicNavigation />` element (optional)

This optional element, when enabled, provides an "alpha" feature that disables dynamic
...
. Add an _entity reference_ (`&securityConfig;`) to the content of
the _web-app_ element. This would cause your external
